extension Collection where Element: Equatable {
  ...
  func between(from: Element, to: Element) -> Self.SubSequence? {
    guard let fromIndex = firstIndex(of: from) else {
      return nil
    }
    guard let toIndex = firstIndex(of: to) else {
      return nil
    }
...
extension Array where Element: Equatable {
  func nearest(to: Element, where condition: (Element) -> Bool) -> Element? {
    guard let currentIndex = firstIndex(of: to) else {
      return nil
    }
